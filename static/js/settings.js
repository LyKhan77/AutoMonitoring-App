...
(function(){
    ...
    async function renderCameraCards(items){
          ...
          <div class="flex items-center gap-4">
            <div class="flex items-center gap-2">
              ...
              <label class="inline-flex items-center cursor-pointer select-none">
                <input type="checkbox" class="sr-only peer" data-toggle-stream="${id}" ${streamEnabled ? 'checked' : ''} />
            ...
            <div class="flex items-center gap-2">
              ...
              <label class="inline-flex items-center cursor-pointer select-none">
                <input type="checkbox" class="sr-only peer" data-toggle-ai="${id}" ${aiRunning ? 'checked' : ''} />
                <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-green-500 relative transition">
                  <span class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition peer-checked:translate-x-5"></span>
                </div>
              </label>
      ...
      // Wire events
      function updateKnob(chk){
        try{
          const wrap = chk.parentElement; // label > input + div(w-11 ...) + span label(optional)
          const track = wrap && wrap.querySelector('div.w-11');
          const knob = track && track.querySelector('span');
          if (knob){ knob.style.transform = chk.checked ? 'translateX(20px)' : 'translateX(0px)'; knob.style.transition = 'transform 150ms ease'; }
          if (track){ track.classList.remove('bg-green-500','bg-gray-200'); track.classList.add(chk.checked ? 'bg-green-500' : 'bg-gray-200'); }
        }catch(e){}
      }
    ...
    if (socket){
      ...
      socket.on('camera_status', (payload)=>{
        ...
        if (payload.ai_running != null){
          ...
          if (chkAI){ chkAI.checked = !!payload.ai_running; chkAI.disabled = false; /* animate */ (function(){ try{ const track = chkAI.parentElement && chkAI.parentElement.querySelector('div.w-11'); const knob = track && track.querySelector('span'); if (knob){ knob.style.transform = chkAI.checked ? 'translateX(20px)' : 'translateX(0px)'; } if (track){ track.classList.remove('bg-green-500','bg-gray-200'); track.classList.add(chkAI.checked ? 'bg-green-500' : 'bg-gray-200'); } }catch(_){ } })(); }
        }
        if (payload.stream_enabled != null){
          const chkStream = camList.querySelector(`input[data-toggle-stream="${camId}"]`);
          if (chkStream){ chkStream.checked = !!payload.stream_enabled; chkStream.disabled = false; /* animate */ (function(){ try{ const track = chkStream.parentElement && chkStream.parentElement.querySelector('div.w-11'); const knob = track && track.querySelector('span'); if (knob){ knob.style.transform = chkStream.checked ? 'translateX(20px)' : 'translateX(0px)'; } if (track){ track.classList.remove('bg-green-500','bg-gray-200'); track.classList.add(chkStream.checked ? 'bg-green-500' : 'bg-gray-200'); } }catch(_){ } })(); }
        }
      });
    }
