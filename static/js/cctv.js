...
(function(){
  ...
  function renderTracking(state){
    ...
    filtered.forEach(it => {
      ...
      if (NC){
        // Also guard here in case filtered conditions change later
        if (it.is_active !== false && prev && prev.is_present === false && it.is_present === true){
          ...
          if (canAlert()){
            NC.pushKeyed(`back:${it.employee_id}`, `${name} back to area after ${formatDuration(secAway, 'after')}`, 60000);
            try{ document.dispatchEvent(new CustomEvent('app:alert-log', { detail: { kind: 'back', employee_id: it.employee_id } })); }catch(_){ }
            // Persist ENTER alert
            try{ fetch('/api/alert_logs', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ employee_id: it.employee_id, alert_type:'ENTER', message: `${name} back to area after ${formatDuration(secAway, 'after')}` }) }); }catch(_){ }
          }
          // Clear EXIT latch so future EXIT can be logged again
          try{ delete _persist.exit[String(it.employee_id)]; savePersist(); }catch(_){ }
        }
        if (it.is_active !== false && prev && prev.is_present === true && it.is_present === false){
          // True transition to EXIT only when prev.present==true -> now false
          // Respect 15-hour mute threshold for EXIT and one-per-away-stretch latch
          const awaySec = Math.max(0, Math.floor((it.seconds_since || 0)));
          const empKey = String(it.employee_id);
          const alreadyExitedTs = _persist.exit[empKey] || 0;
          const sincePrev = nowSec() - (Number(alreadyExitedTs)||0);
          if (awaySec < AWAY_MUTE_THRESHOLD_SEC && (!alreadyExitedTs || sincePrev >= minIntervalSec())){
            if (canAlert()){
              const name = it.name || `Employee ${it.employee_id}`;
              NC.pushKeyed(`seen:${it.employee_id}`, `${name} out of area since ${durationText(it)}`, 60000);
              try{ document.dispatchEvent(new CustomEvent('app:alert-log', { detail: { kind: 'seen', employee_id: it.employee_id } })); }catch(_){ }
              // Persist EXIT alert
              try{ fetch('/api/alert_logs', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ employee_id: it.employee_id, alert_type:'EXIT', message: `${name} out of area since ${durationText(it)}` }) }); }catch(_){ }
            }
            // Latch EXIT to avoid duplicates until ENTER occurs (even if suppressed, keep latch)
            try{ _persist.exit[empKey] = nowSec(); savePersist(); }catch(_){ }
          }
        }
      }
