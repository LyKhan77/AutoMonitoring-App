...
(function(){
  ...
  const NotificationCenter = (function(){
    const state = { history: [], unread: 0, lastKeys: new Map(), recentMsgs: new Set() };
    ...
    function render(){ if (!elNotifList) return; const parts=[]; state.history.forEach(h=>parts.push(formatHistoryRow(h))); elNotifList.innerHTML = parts.length ? parts.join('') : '<div class="px-4 py-3 text-sm text-gray-500">No notifications</div>'; if (elNotifBadge){ const c=state.unread; if (c>0){ elNotifBadge.textContent = '!'; elNotifBadge.classList.remove('hidden'); } else { elNotifBadge.classList.add('hidden'); } } }
    ...
    function pushKeyed(key,msg,ttlMs){ const now=Date.now(); const last=state.lastKeys.get(key)||0; const ttl = (ttlMs||60000); if (now-last < ttl) return; state.lastKeys.set(key, now); saveLastKeys(); pushRaw(msg); }
    loadHistory(); loadLastKeys(); setTimeout(render,0);
    return { render, push: pushRaw, pushKeyed, clear(){
      // wipe history and unread, and remove storage to avoid reloading on refresh
