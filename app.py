...
try:
    ...
    cv2 = None
try:
    # Prefer the consolidated StreamRTSP module
    from StreamRTSP import RTSPFrameSource
except Exception:
    try:
        # Backward compatibility
        from module_AI_rtsp import RTSPFrameSource
    except Exception:
        RTSPFrameSource = None
                ...
                if cam_id is not None:
                    ...
                    if alert_type == 'ENTER':
                        ...
                        if delay_sec and delay_sec > 0:
                            threading.Thread(target=lambda: (time.sleep(min(30, delay_sec)), _do_last_out_save()), daemon=True).start()
                        else:
                            _do_last_out_save()
            except Exception:
                pass
